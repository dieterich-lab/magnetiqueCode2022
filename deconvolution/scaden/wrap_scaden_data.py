...
execs = '/prj/MAGE/analysis/deconvolution/scaden/scaden_data.py'
...
def main():
    ...
    samples = {}
    filen = {}
    for fname in glob.glob(os.path.join(args.scDir, '*')):
        scData = os.path.basename(fname).split('-')[0]
        filen[scData] = fname
        with lp.connect(fname) as ds:
            samples[scData] = list(set(ds.ca['Individual']))
            
    for scData, sampleList in samples.items():
        #for sample in sampleList: # too many open connections!
        cmd = "{} {} {} {} --samples {} --fmt {}".format(
            execs,
            scData,
            filen[scData],
            args.locDir,

runMethod <- function(input.scores = input.scores, 
  ...
  if(is.null(all_constraints)){
    ...
    # save(allC, file = "../data_set/allC.RData")
  } else {
    load(file = all_constraints)
  }
  
  res <- computeILP_Single(variables = variables,
                           background.network = bn$background.network, 
  ...
  return(res)
  save(res, file = paste0("res_", ii, ".RData"))
